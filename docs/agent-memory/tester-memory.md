...
- 第五轮“主题一致性收口”回归中，优先跑 `npm run build` 再跑 `npm run test:e2e:with-screenshots`，可快速确认样式替换未引入 Astro 编译问题；本轮 16/16 通过。
- 当页面切换到 token 类（`text-primary/text-secondary/border-default`）后，原有行为断言基本不受影响，说明建议将视觉改造与交互断言解耦，避免视觉调整频繁触发无关失败。
